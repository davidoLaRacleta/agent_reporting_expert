...
def register(registry: ToolRegistry, manager: PresentationManager) -> None:
    """Register chart-related tools onto ``registry``."""
    ...
    def add_chart(
        ...
        if xl_chart_type is None:
            raise ToolExecutionError(
                f"Unknown chart_type '{chart_type}'. Supported: {sorted(_CHART_TYPES)}."
            )
        if not categories:
            raise ToolExecutionError("categories must be a non-empty list.")
        if not series:
            raise ToolExecutionError(
                "series must be a non-empty mapping of series name to values."
            )
        ...
        parameters={
            ...
            "properties": {
                ...
                "categories": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Labels along the category axis, e.g. quarters or product names.",
                },
                "series": {
                    "type": "object",
